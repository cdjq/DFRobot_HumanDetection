...
uint8_t DFRobot_HumanDetection::configWorkMode(eWorkMode mode){
    uint8_t data = mode;
    uint8_t buf[10];
    if(getData(0x02,0x08,1,&data,buf) != 0){
        return 1;
    }
    delay(30000);//等待切换模式启动
    data = 0;
    while(getData(0x80,0x00,1,&data,buf) != 0){//关闭人体主动上报
        delay(100);
    }
    delay(100);
    if(mode == eSleepMode){
...
uint32_t DFRobot_HumanDetection::unmannedTime(void){
    uint8_t readBuf[15];
    uint8_t data = 0x0f;
    uint32_t ret = 0;
...
uint32_t DFRobot_HumanDetection::staticResidencyTime(void){
    uint8_t readBuf[15];
    uint8_t data = 0x0f;
    uint32_t ret = 0;
